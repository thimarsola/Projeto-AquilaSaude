...
return rgb};var isDefined=function isDefined(variable){return typeof variable!=='undefined'};var uniqueArray=function uniqueArray(arr){var result=[];for(var i in arr){if(result.indexOf(arr[i])===-1){result.push(arr[i])}}
...
if(!isDefined(args[0])){error('SweetAlert2 expects at least 1 attribute!');return!1}
...
getContainer().scrollTop=0})};sweetAlert$1.isVisible=function(){return!!getPopup()};sweetAlert$1.queue=function(steps){queue=steps;var resetQueue=function resetQueue(){queue=[];document.body.removeAttribute('data-swal2-queue-step')};var queueResult=[];return new Promise(function(resolve,reject){(function step(i,callback){if(i<queue.length){document.body.setAttribute('data-swal2-queue-step',i);sweetAlert$1(queue[i]).then(function(result){if(isDefined(result.value)){queueResult.push(result.value);step(i+1,callback)}else{resetQueue();resolve({dismiss:result.dismiss})}})}else{resetQueue();resolve({value:queueResult})}})(0)})};sweetAlert$1.getQueueStep=function(){return document.body.getAttribute('data-swal2-queue-step')};sweetAlert$1.insertQueueStep=function(step,index){if(index&&index<queue.length){return queue.splice(index,0,step)}
return queue.push(step)};sweetAlert$1.deleteQueueStep=function(index){if(isDefined(queue[index])){queue.splice(index,1)}};sweetAlert$1.close=sweetAlert$1.closePopup=sweetAlert$1.closeModal=sweetAlert$1.closeToast=function(onComplete){var container=getContainer();var popup=getPopup();if(!popup){return}
...
var testEl=document.createElement('div');var transEndEventNames={'WebkitAnimation':'webkitAnimationEnd','OAnimation':'oAnimationEnd oanimationend','animation':'animationend'};for(var i in transEndEventNames){if(transEndEventNames.hasOwnProperty(i)&&isDefined(testEl.style[i])){return transEndEventNames[i]}}
return!1}();var resetPrevState=function resetPrevState(){if(states.previousActiveElement&&states.previousActiveElement.focus){var x=window.scrollX;var y=window.scrollY;states.previousActiveElement.focus();if(isDefined(x)&&isDefined(y)){window.scrollTo(x,y)}}};var measureScrollbar=function measureScrollbar(){var supportsTouch='ontouchstart' in window||navigator.msMaxTouchPoints;if(supportsTouch){return 0}
var scrollDiv=document.createElement('div');scrollDiv.style.width='50px';scrollDiv.style.height='50px';scrollDiv.style.overflow='scroll';document.body.appendChild(scrollDiv);var scrollbarWidth=scrollDiv.offsetWidth-scrollDiv.clientWidth;document.body.removeChild(scrollDiv);return scrollbarWidth};var injectCSS=function injectCSS(){var css=arguments.length>0&&arguments[0]!==undefined?arguments[0]:'';if(isNodeEnv()){return!1}
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){$('#form').submit(function(){$.ajax({url:'https://localhost/aquilasaude.com.br/wp-content/themes/aquila/source/Support/Sender.php',type:'POST',data:$('#form').serialize(),success:function(data){$('.form__row__form__status').html(data)}});return!1})});$(function(){$("#button").click(function(){$(".form__row__form__status--loading").removeClass("d-none")})})