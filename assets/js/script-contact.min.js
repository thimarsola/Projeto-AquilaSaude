...
return jQuery});$(function(){$('.header__navbar__section__toggle').on('click',function(){const clicks=$(this).data('clicks');const header=$('.header');if(clicks){$('body').css('overflow','auto')}else{$('body').css('overflow','hidden')}
$(this).not('a').data("clicks",!clicks);header.toggleClass('header--active');if(header.hasClass('header--active')){$('body').css('overflow','hidden');$('.menu li a[href]').on('click',function(){$('.header').removeClass('header--active');$('body').css('overflow','auto')})}})});$(function(){$('.menu-item-has-children > a').removeAttr("href").click(function(e){e.preventDefault()});$('.menu-item-has-children').on('click',function(e){e.stopPropagation();$(this).toggleClass('rotate');$('.sub-menu').stop().slideToggle(500);if($('.menu-item-has-children').hasClass('rotate')){$(document).on('click',function(){$('.menu-item-has-children').removeClass('rotate');$('.sub-menu').slideUp(500)})}})});$(function(){$(window).on("scroll",function(){if($(window).scrollTop()){$('.header').addClass('header--sticky')}else{$('.header').removeClass('header--sticky')}})});/*!
 * sweetalert2 v7.0.8
 * Released under the MIT License.
 */
...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){$('#form').submit(function(){$.ajax({url:'https://aquilasaude.com.br/wp-content/themes/aquila/source/Support/Sender.php',type:'POST',data:$('#form').serialize(),success:function(data){$('.form__row__form__status').html(data)}});return!1})});$(function(){$("#button").click(function(){$(".form__row__form__status--loading").removeClass("d-none")})})